#!/usr/bin/env python3
"""
Proxmox MCP Server - LXC Application
A beginner-friendly MCP server for Proxmox integration running in LXC
"""
...
load_dotenv('/opt/proxmox-mcp-server/config.env')
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/opt/proxmox-mcp-server/logs/app.log'),
...
app = FastAPI(
    title="Proxmox MCP Server (LXC)",
    description="MCP Server for Proxmox Virtual Environment running in LXC",
    version="1.0.0"
)
...
async def startup_event():
    """Application startup"""
    logger.info("Starting Proxmox MCP Server (LXC)...")

    # Create logs directory
    Path("/opt/proxmox-mcp-server/logs").mkdir(exist_ok=True)
...
async def root():
    """Root endpoint"""
    return {
        "message": "Proxmox MCP Server is running in LXC",
        "deployment": "lxc",
        "version": "1.0.0"
    }
...
@app.get("/proxmox/node/{node}/containers")
async def get_containers(node: str):
